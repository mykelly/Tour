...
/*
 * Copyright (C) 2011 Patrik Aerfeldt
 * 
...
public class TourViewFlow extends AdapterView<Adapter> {
	...
	private static final int SNAP_VELOCITY = 150;// 大于该移动X轴的距离值时切换下一张图
	...
	private long timeSpan = 4000;// 4秒切换下一张
	...
	public TourViewFlow(Context context) {
		super(context);
		mSideBuffer = 3;
		init();
	}

	public TourViewFlow(Context context, int sideBuffer) {
		super(context);
		mSideBuffer = sideBuffer;
		init();
	}

	public TourViewFlow(Context context, AttributeSet attrs) {
		super(context, attrs);
	 ...
	 * 用于判断轮播是否停止
	 * 
	 * @return true:关闭了自动播放，需要重新启动 false:不需要开启
	 */
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			// action_up时，重新发延迟消息以恢复自动播放，如果已存在该消息则不发送
			if (handler != null && !handler.hasMessages(0)) {
				Message message = handler.obtainMessage(0);
				handler.sendMessageDelayed(message, timeSpan);
			}
	 ...
	 * @param flowIndicator
	 */
	public void setFlowIndicator(TourFlowIndicator flowIndicator) {
