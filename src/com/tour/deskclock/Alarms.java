...
public class Alarms {
    ...
    private static ContentValues createContentValues(Alarm alarm) {
        ...
        values.put(Alarm.Columns.ISDELETE, alarm.isdelete ? 1 : 0);
        return values;
    }
        ...
        if (cur.moveToFirst()) {
            do {
                ...
                // non-zero, check if the time is before now.
                 ...
                 String nowtime=now+"";
                 boolean isflag=nowtime.length()>time.length()?true:false;
               /* if (alarm.time != 0 &&isflag&& alarm.time*1000 < now) {
                    enableAlarmInternal(context, alarm, false);
                }else*/ if(alarm.time != 0 && alarm.time < now){
                	enableAlarmInternal(context, alarm, false);
                }
     ...
     * boot.
     */
    public static void disableExpiredAlarms(final Context context,int id) {
        Cursor cur = getFilteredAlarmsCursor(context.getContentResolver());
            ...
            do {
                ...
                TTLog.s("id====="+id);
                 if (id==alarm.id) {
                	 ...
                	 resolver.update(ContentUris.withAppendedId(
                             Alarm.Columns.CONTENT_URI, id), values, null, null);
				}
            } while (cur.moveToNext());
        }
        cur.close();
